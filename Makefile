...
# BUILD #
#########
.PHONY: develop build build-systemc install dependencies-linux dependencies-macos dependencies-win copy-systemc copy-systemc-win
...
build-systemc:  ## build systemc
	git submodule update --init --recursive
	cmake -B build src -DCMAKE_CXX_STANDARD=17 -DCMAKE_INSTALL_PREFIX=systemc
	cmake --build build --config Release -j 3
	cmake --install build

copy-systemc:  ## copy systemc to bin
...
lint-py:  ## lint python with ruff
	python -m ruff check systemc
	python -m ruff format --check systemc
...
fix-py:  ## autoformat python code with ruff
	python -m ruff check --fix systemc
	python -m ruff format systemc
...
test:  ## run python tests
	python -m pytest -v systemc/tests

coverage:  ## run tests and collect test coverage
	python -m pytest -v systemc/tests --cov=systemc --cov-report term-missing --cov-report xml
